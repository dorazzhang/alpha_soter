...
class LLMOptimizer:
    ...
    def optimize(self, arch_yaml, problem_yaml, parent_map, example_map, score, regenerate = False, iterate = False, inspirations = None, error_message = None, prompt = None):
        ...
        if regenerate:
            ...
            user_prompt = (
                ...
                "3. product of all factors of a dimension is not equal to the dimension size of the workload. Increase or decrease factors, depending on if the current product is less than or greater than the dimension size, of that dimension at different levels until they all multiply to the instance size\n\n"
                "Previous map.yaml with error:\n"
                f"{parent_map}\n\n" 
                "Previous Prompt (Follow this as a guidance):\n"
                f"{prompt}"
            )
        ...
        if iterate:
            ...
            user_prompt = (
                ...
                f"{parent_map}\n\n"
                f"Current evaluation score to minimize (calculated as edp):\n{score}\n\n"
                "Here are some other alternative mappings for inspiration:\n"
                f"{inspiration_block}\n\n"

                "Task:\n"
                "Generate an improved map.yaml to minimize edp"
                ...
                "Verification checklist (with calculation examples using the example map attached):\n\n"
                "Mapping used for calculation examples:\n"
                f"{example_map}\n\n"
                ...
                "(e.g., 16 * ((14-1)*1 + 1 + 1*(1-1)) * ((14-1)*1 + 1 + 1*(1-1)) * 2 * 1 = 6272)\n\n"
                ...
                "    Output tile = P_eff * Q_eff * K_eff * N_eff * H_eff (e.g., 14 * 14 * 16 * 16 * 1 = 50176)\n"
                "    Total = input + output = 12544 + 50176 = 62720\n\n"
                ...
                "    P_eff = 1*14*1*1*1*1*1*1 = 14 == Instance size = 14\n"
